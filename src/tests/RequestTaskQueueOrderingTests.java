...
public class RequestTaskQueueOrderingTests {
	...
	public void testUnestimatedSameStartTimeTasks() {
		...
		FutureRequestTask<RequestTaskBase, Void> task1 = createTestTask(null,
				timestamp, server);
		FutureRequestTask<RequestTaskBase, Void> task2 = createTestTask(null,
				timestamp, server);
	...
	public void testEstimatedSameStartTimeTasks() {
		...
		FutureRequestTask<RequestTaskBase, Void> task1 = createTestTask(
				new RequestCacheTests.FakeHttpRequest("GET", "/"), timestamp,
				server);
		FutureRequestTask<RequestTaskBase, Void> task2 = createTestTask(
				new RequestCacheTests.FakeHttpRequest("GET", "/"), timestamp,
				server);
	...
	public void testFewEstimatedDifferentlyWithDifferentStartTimeTasks() {
		...
		FutureRequestTask<RequestTaskBase, Void> task2 = createTestTask(
				new RequestCacheTests.FakeHttpRequest("GET", "/path2"),
				new Date(timestamp.getTime() - 1000), server);
		FutureRequestTask<RequestTaskBase, Void> task3 = createTestTask(
				new RequestCacheTests.FakeHttpRequest("GET", "/path3"),
				new Date(timestamp.getTime() - 1000), server);
		FutureRequestTask<RequestTaskBase, Void> task4 = createTestTask(
				new RequestCacheTests.FakeHttpRequest("GET", "/path4"),
				new Date(timestamp.getTime() - 500), server);

		Object[] tasks = new Object[] { task1, task2, task3, task4 };
	...
	private static FutureRequestTask<RequestTaskBase, Void> createTestTask(
			IHttpRequest request, Date timestamp, Server server) {
		if (timestamp == null) {
			timestamp = new Date();
		}
		if (request == null) {
		...
		return new FutureRequestTask<RequestTaskBase, Void>(result);
	}
